...
module.exports = function(http){
  ...
  io.on('connection', function(socket){
    ...
    socket.on('join-channel', function(username, room){
        ...
        console.log("[server] -> user "+ username+" entre dans la chambre "+room);
    });

    socket.on('back-home', function(username, room){
      console.log('[server] -> user '+username+" sors de la chambre "+room);
      io.to(room).emit("back-home-message", username);
      socket.leave(room);
    });

    socket.on('create_room', function(roomName, roomDesc){
      console.log("[[Server]] -> roomName :"+roomName+" | roomDesc :"+roomDesc);
    ...
    socket.on('sendMessage', function(username, room, msg){
        console.log("[server] room"+room+" -> "+username+" > "+ msg );
